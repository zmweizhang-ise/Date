...
private void Update()
    {
        ...
        {
            Debug.Log(hitCollider.gameObject.layer); //find rock by layer (rock is layer 17)
        }
        ...
        switch (state) //going to add more features later
        {
            ...
            case STATE.Collect: // detects rocks and collect
                if (_rockDetected == false || _rock ==  null)
                {
                    state = STATE.Patrol;
                }
                else
                {
                    _agent.destination = _rock.transform.position;
                }
                break;
            case STATE.Follow: // goes to player
